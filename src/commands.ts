...
export function register(
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      repeatLastUsedSelectionId,
      repeatLastUsedSelection
    ),
    ...
    vscode.commands.registerCommand(
      toggleRecordingMacroId,
      toggleRecordingMacro
    ),
    vscode.commands.registerCommand(
      cancelRecordingMacroId,
  ...
  macroStatusBar.backgroundColor = new vscode.ThemeColor(
...
async function runCapture(str: string) {
  ...
  if (captureCommand === undefined) {
    vscode.window.showErrorMessage(
      "Unexpected missing command for capture string."
    );
  } else {
 ...
 */
async function runActionForKey(
  key: string,
  mode: string = keyMode,
  ...
  if (newmode !== Insert) {
    if (!typeSubscription) {
      try {
        ...
        vscode.window.showErrorMessage(
          "Another extension is overwritting the 'type' command. (E.g. VSCodeVim). ModalKeys will not behave properly."
        );
      }
    }
...
export async function enterInsert() {
  enterMode("insert");
}
...
export async function enterMode(args: string | EnterModeArgs) {
  ...
  if (editor?.document.uri)
    editorModes[editor?.document.uri.toString()] = visualFlag
      ? Visual
      : newMode;
  const enterHook =
    modeHooks[keyMode]?.enter || modeHooks["__default__"]?.enter;
 ...
 * status bar items.
 */
export function updateCursorAndStatusBar(
  ...
  if (editor) {
    ...
    if (help) sec = `${sec}    ${help}`;
    if (matchStatusText) {
      if (sec.trim() == "") sec = matchStatusText;
      else matchStatusText = "";
    }
...
async function toggleSelection(): Promise<void> {
  if (isSelecting()) {
    ...
    if (editor) {
      editor.selections = editor.selections.map(
        (x) => new vscode.Selection(x.active, x.active)
      );
    }
    enterMode(Normal);
  } else {
...
function isSelecting(
  ...
  return (
    keyMode === Normal &&
    (visualFlag || (editor ? editor.selections.some((s) => !s.isEmpty) : false))
  );
}
...
function* searchMatches(
  ...
  if (args.regex) {
    ...
    matchesFn = (line, offset) =>
      regexMatches(matcher, line, !args.backwards, offset);
  } else {
...
async function search(args: SearchArgs | string): Promise<void> {
  ...
  if (typeof args == "object") {
    ...
    state.args.highlightMatches =
      args.highlightMatches === undefined ? true : args.highlightMatches;
    ...
    if (
      !state.args.acceptAfter ||
      state.string.length >= state.args.acceptAfter
    )
      await acceptSearch(editor, state);
  }
}
...
function updateSearchHighlights(event?: vscode.ConfigurationChangeEvent) {
  if (!event || event.affectsConfiguration("modalkeys")) {
    ...
    let highlightBackground = config.get<string>(
      "searchOtherMatchesBackground"
    );
    ...
    searchDecorator = vscode.window.createTextEditorDecorationType({
      ...
      borderColor:
        matchBorder || new vscode.ThemeColor("editor.findMatchBorder"),
      borderStyle: "solid",
    });
...
function positionSearch(
  ...
  if (offset !== 0) {
    ...
    return new vscode.Selection(
      args.selectTillMatch ? sel.anchor : newpos,
      newpos
    );
  }
  return sel;
...
function selectBetween(args: SelectBetweenArgs) {
  ...
  if (editor) {
    ...
    ed.selections = ed.selections.map((sel: vscode.Selection) => {
      ...
      let fromMatches = searchMatches(
        ed.document,
        start,
        undefined,
        args.from,
        { ...args, backwards: true, wrapAround: false }
      );
      let toMatches = searchMatches(ed.document, end, undefined, args.to, {
        ...args,
        backwards: false,
        wrapAround: false,
      });
      ...
      while (!to.done && !from.done) {
        ...
        if (
          !betweenSel.start.isEqual(sel.start) ||
          !betweenSel.end.isEqual(sel.end)
        )
          break;
      ...
      if (to.done || from.done) {
        ...
        if (sel.isEmpty || !sel.isReversed) return betweenSel;
        else return new vscode.Selection(betweenSel.active, betweenSel.anchor);
      }
    });
  }
}
...
async function replayMacro(args?: { register?: string }): Promise<void> {
  let [nestedState, seq, mode] = keyState.macroReplayState(
  ...
  let entries = <any>(
    JSON.parse(
      new TextDecoder("utf-8")
        .decode(await fs.readFile(presetsFiles))
        .toString()
    )
  );
...
async function importFile(uri: vscode.Uri) {
  ...
  try {
    ...
    if (uri.fsPath.match(/jsonc?$/)) js = `(${js})`;
    ...
    else config.update("keybindings", preset.keybindings, true);
    ...
    vscode.window.showInformationMessage("ModalKeys: Keybindings imported.");
  } catch (e) {
    vscode.window.showWarningMessage(
      "ModalKeys: Bindings not imported." + `\n ${e}`
    );
  }
}
...
async function importPresets(folder?: string) {
  ...
  let choice = await vscode.window.showQuickPick(presets, {
    placeHolder:
      "Warning: Selecting a preset will override current " +
...
function checkExtensions(extension_ids: string[] | undefined) {
  ...
  for (const id of extension_ids) {
    if (!vscode.extensions.getExtension(id)) {
      ...
      vscode.window
        .showWarningMessage(
          `The current ModalKeys keybindings
                needs the extension (\`${id}\`), but it may not be installed.`,
          view,
          install
        )
        .then((sel) => {
...
async function toggleKeymap() {
  if (!docKeymap?.visible()) {
    let editor = vscode.window.activeTextEditor;
    await vscode.commands.executeCommand(
      "workbench.view.extension.modalKeyBindingView"
    );
